...
| walking `Gait`<br/>(Former name was "isWalking") | boolean<br/>(Former type was Int) | Number | Detection of one step when heel touches the ground (flagged 0.15~0.25s after detection) | 0/false: no step<br/>1/true: step |
| noiseStatus | boolean<br/>(Former type was Int) | Number | Integer value representing the noise condition of the electrode | 0/false: low noise<br/>1/true: high noise |
...
※"Normalized" and "sub-index" may not be recorded.
...
|blinkIntervalMean / bki_av `Static`|Number(float)|Blink interval mean(s) | 0.00-60.00 |
...
The following indicators were generated by the legacy JINS MEME app.
