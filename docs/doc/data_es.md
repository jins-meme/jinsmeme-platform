...
|:--|:--|--|:--:|
|date / date|String|計測日時で、15秒前からこのタイムスタンプまでの期間のsummerizeが行われます。15秒の区切りは、計測ボタンを押したタイミングではなく、接続を開始したタイミングから15秒づつで、測定開始から終了までにサマライズが行われたポイントのデータが記録されます。|1970-01-01T00:00:00Z - 2099-12-31T23:59:59|
|stepCount / stp `歩行指標`|Number(int)|歩数| 0-255 |
|stepCadence / cad `歩行指標`|Number(float)|ケイデンス(ピッチ)| 0-255 |
|isStill / isl|Boolean / Number(int)|静止（装着してない）判定| true: 非装着（静止） false: 装着中（非静止）|
|noiseTime / nis_time|Number(float)|ノイズ時間|0.00 - 15.00|
|isValid / vld|Boolean / Number(int)|静止状態のデータ有効性（ノイズ3秒以下かつ歩数5歩以下）| true: 有効 false: 無効|
|xMean / tl_xav|Number(float)|傾き平均X (度) | -180.00-180.00 |
|xSD / tl_xsd|Number(float)|傾き標準偏差X (度) | 0-655.36 |
|yMean / tl_yav|Number(float)|傾き平均Y (度) | -180.00-180.00 |
|ySD / tl_ysd|Number(float)|傾き標準偏差Y (度) | 0-655.36 |
|pitchOnewayCount / hm_po|Number(int)|頭部運動縦回数| 0-255 |
|pitchRoundCount / hm_pr|Number(int)|ゆっくりな首の連続傾斜回数（前後）| 0-255 |
|yawOnewayCount / hm_yo|Number(int)|頭部運動横回数| 0-255 |
|yawRoundCount / hm_yr|Number(int)|ゆっくりな首の連続傾斜回数（左右）| 0-255 |
|xRightStepAmplitude / sa_xr `歩行指標`|Number(float)|歩行振動X（cm,右足）| 0.00-16.00 |
|xLeftStepAmplitude / sa_xl `歩行指標`|Number(float)|歩行振動X（cm,左足）| 0.00-16.00 |
|yRightStepAmplitude / sa_yr `歩行指標`|Number(float)|歩行振動Y（cm,右足）| 0.00-16.00 |
|yLeftStepAmplitude / sa_yl `歩行指標`|Number(float)|歩行振動Y（cm,左足）| 0.00-16.00 |
|zRightStepAmplitude / sa_zr `歩行指標`|Number(float)|歩行振動Z（cm,右足）| 0.00-16.00 |
|zLeftStepAmplitude / sa_zl `歩行指標`|Number(float)|歩行振動Z（cm,左足）| 0.00-16.00 |
|zLeftStepAmplitudeCal / sa_zrc `歩行指標`|Number(float)|歩行振動Z補正有（cm,右足）| 0.00-20.00 |
|zLeftStepAmplitudeCal / sa_zlc `歩行指標`|Number(float)|歩行振動Z補正有（cm,左足）| 0.00-20.00 |
|maxRightStepAcceleration / st_r `歩行指標`|Number(float)|最大着地強度平均 (G, 右足)| 0.00-8.00 |
|maxLeftStepAcceleration / st_l `歩行指標`|Number(float)|最大着地強度平均 (G, 左足)| 0.00-8.00 |
|sleepScoreStandard / sc_slp_std `静止指標`|Number(float)|低覚醒スコア(通常時)、まばたき強さ・速度と一部まばたき間隔から「目がトロンとなっている状態」を指標化したもの|有効時: 0(覚醒高い)-100(眠い)、無効: -1|
|sleepScore / sc_slp `静止指標`|Number(float)|低覚醒スコア(運転時)、運転時の姿勢(まっすぐ前を向いている状態)でクレンジングし、精度を高めたもの|有効時: 0(覚醒高い)-100(眠い)、無効: -1|
...
|Logger/API項目名|型(API/CSV)|説明|値の範囲|
|:---|:---:|:---|:---:|
|date|String|タイムスタンプ、CSVのみ。区切りのタイミングはJINS MEMEのRTCにセットされた分単位で、直前1分間のデータがサマライズされ測定開始から終了までにサマライズが行われたポイントのデータが記録されます。|1970-01-01T00:00:00Z - 2099-12-31T23:59:59|
|ut|Number(int)|UNIX TIME、APIのみ|0-4294967296|
...
|eyeMoveHorizontal / ems_rl `静止指標`|Number(int)|視線移動小回数(左右)|0-255|
|eyeMoveVertical / ems_ud `静止指標`|Number(int)|視線移動小回数(上下)|0-255|
|eyeMoveBigHorizontal / eml_rl `静止指標`|Number(int)|視線移動大回数(左右)|0-255|
|eyeMoveBigVertical / eml_ud `静止指標`|Number(int)|視線移動大回数(上下)|0-255|
|headMoveVerticalCount / hm_po|Number(int)|頭部運動回数縦|0-255|
|headMoveHorizontalCount / hm_yo|Number(int)|頭部運動回数横|0-255|
|walkingVibrationRightX / sa_xr `歩行指標`|Number(float)|歩行振動X (cm, 右足)|0.00-16.00|
|walkingVibrationLeftX / sa_xl `歩行指標`|Number(float)|歩行振動X (cm, 左足)|0.00-16.00|
|walkingVibrationRightY / sa_yr `歩行指標`|Number(float)|歩行振動Y (cm, 右足)|0.00-16.00|
|walkingVibrationLeftY / sa_yl `歩行指標`|Number(float)|歩行振動Y (cm, 左足)|0.00-16.00|
|walkingVibrationRightZ / sa_zr `歩行指標`|Number(float)|歩行振動Z (cm, 右足)|0.00-16.00|
|walkingVibrationLeftZ / sa_zl `歩行指標`|Number(float)|歩行振動Z (cm, 左足)|0.00-16.00|
|landingStrengthRightMaxAvg / st_r `歩行指標`|Number(float)|最大着地強度平均 (G, 右足)|0.00-8.00|
|landingStrengthLeftMaxAvg / st_l `歩行指標`|Number(float)|最大着地強度平均 (G, 左足)|0.00-8.00|
...
|highSpeedStepsNum / stp_fst `歩行指標`|Number(int)|歩行時歩数（高速 280-370ms）|0-255|
|middleSpeedStepsNum / stp_mid `歩行指標`|Number(int)|歩行時歩数（中速 380-440ms）|0-255|
|lowSpeedStepsNum / stp_slw `歩行指標`|Number(int)|歩行時歩数（低速 450-590ms）|0-255|
|ultraLowSpeedStepsNum / stp_vsl `歩行指標`|Number(int)|歩行時歩数（超低速 600-1000ms）|0-255|
|nptAvgWeak / lc_npt_av `静止指標` `クレンジング弱`|Number(float)|NPT（実効まばたき速度）平均|-0.256 - 0.256|
|weakBlinkSpeedAvg / lc_bkw_av `静止指標` `クレンジング弱`|Number(int)|まばたき速度平均(mSec)|50-306|
|weakBlinkSpeedStd / lc_bkw_sd `静止指標` `クレンジング弱`|Number(float)|まばたき速度標準偏差(mSec)|0-51.2|
|weakBlinkStrengthAvg / lc_bkh_av `静止指標` `クレンジング弱`|Number(int)|まばたき強度平均(uV-equiv)|0-512|
|weakBlinkStrengthStd / lc_bkh_sd `静止指標` `クレンジング弱`|Number(float)|まばたき強度標準偏差(uV-equiv)|0-51.2|
